import {ChromelessPlayer, MediaTrack, MediaType, Quality, Request, TypedSource} from 'theoplayer';
...
export class CMCDCollector {
    ...
    private handleCurrentSourceChange_ = (event: any) => {
        const currentSource: TypedSource | undefined = event.currentSource;
        if (!currentSource) {
    ...
    /**
     * Collects all of the CMCD parameters as supported by the connector depending on the provided {@link Request}.
...
function calculateRequestedMaximumThroughput(
    ...
    const requestedMaximumThroughputRoundedToTheNextMultipleOf100Kbps = ((requestedMaximumThroughputInKbps / 100) + 1) * 100;
    return requestedMaximumThroughputRoundedToTheNextMultipleOf100Kbps;
}
